    ...
    while t_end > time.time() :
        ...
        try:
            ...
            email="""
                    ...
                    Il n'arrive pas a acceder  a l'adresse %s. Il y a de grandes chances que cela provienne d'une
                    surchauffe du raspberry (celui-ci etant connecte au serveur en local). Il vous est conseille de redemarrer 
                    le raspberry le plus rapidement possible (boutton rouge domino). Le probleme a eu lieu en %s pour l'utilisateur %s.
                    Resume de l'exception : %s
...
def log_thread(q_log):
    ...
    log_email_f = open("logs/email.log","a")
